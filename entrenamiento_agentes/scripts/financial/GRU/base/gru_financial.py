...
GRU base con datos puramente financieros (Yahoo Finance).
Sin noticias, sin Reddit.
Arquitectura: 2 capas GRU + Dropout. Idéntica al LSTM pero con GRU.

GRU vs LSTM: menos parámetros (no tiene celda de memoria separada),
entrena más rápido y suele generalizar igual o mejor con datasets pequeños.

Output:
  scripts/financial/outputs/GRU/gru_financial_results.csv
  scripts/financial/outputs/GRU/gru_financial_plot.png
"""
...
OUTPUT_CSV  = "scripts/financial/outputs/GRU/gru_financial_results.csv"
OUTPUT_PLOT = "scripts/financial/outputs/GRU/gru_financial_plot.png"

TIME_STEPS   = 15
EPOCHS       = 100
BATCH_SIZE   = 32
RANDOM_STATE = 42
STEP_WF      = 20   # reentrenar cada N días en walk-forward (GRU es más rápido que LSTM)
...
def build_model(n_features, time_steps):
    """GRU con la misma arquitectura que el LSTM para comparación directa."""
    model = Sequential([
...
def walk_forward(X, y, threshold=0.50, min_train=None, step=STEP_WF):
    """Walk-forward para GRU: reentrenar cada `step` días."""
...
def main():
    ...
    print("=" * 60)
    print(f"GRU BASE — DATOS FINANCIEROS  (time_steps={TIME_STEPS})")
    print("=" * 60)
    ...
    print(f"\nSecuencias train: {X_tr_seq.shape}  ({X_tr_seq.shape[0]} muestras)")
    ...
    axes[0].set_title("GRU — Loss durante entrenamiento", fontweight="bold")
    ...
    print(f"\n{'='*60}\nRESUMEN GRU vs LSTM\n{'='*60}")
    print(f"Test  → acc={res_base['accuracy']:.4f}  auc={res_base['roc_auc']:.4f}")
    print(f"WF    → acc={wf['accuracy']:.4f}  auc={wf['roc_auc']:.4f}")
    print(f"Parámetros GRU: {model.count_params():,}  (LSTM tendría ~{int(model.count_params()*4/3):,})")
