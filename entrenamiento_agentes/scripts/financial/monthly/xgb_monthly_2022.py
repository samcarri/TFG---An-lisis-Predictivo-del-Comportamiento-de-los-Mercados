...
XGBoost para predicción a 1 mes (21 días hábiles) con datos financieros.

Output:
  scripts/financial/outputs/monthly/xgb_monthly_2022_results.csv
  scripts/financial/outputs/monthly/xgb_monthly_2022_plot.png
"""
...
OUTPUT_CSV  = "scripts/financial/outputs/monthly/xgb_monthly_2022_results.csv"
OUTPUT_PLOT = "scripts/financial/outputs/monthly/xgb_monthly_2022_plot.png"
MODEL_PATH  = "trained_models/xgb_monthly_2022_model.joblib"
SCALER_PATH = "trained_models/xgb_monthly_2022_scaler.joblib"
...
def main():
    ...
    print("=" * 60)
    print(f"XGBOOST MONTHLY 2022 — PREDICCIÓN A {PREDICTION_HORIZON} DÍAS")
    print("=" * 60)
    ...
    axes[0].barh(range(len(imp)), imp["importance"], color="#e67e22", edgecolor="white")
    axes[0].set_yticks(range(len(imp))); axes[0].set_yticklabels(imp["feature"], fontsize=8)
    axes[0].invert_yaxis(); axes[0].set_xlabel("Importancia")
    axes[0].set_title(f"XGBoost Monthly ({PREDICTION_HORIZON}d) — Feature Importance", fontweight="bold")
    ...
    model_final = xgb.XGBClassifier(
        ...
        subsample=0.8, colsample_bytree=0.8, min_child_weight=3,
        gamma=0.1, reg_alpha=0.1, reg_lambda=1.0,
        scale_pos_weight=round(n_neg_all / n_pos_all, 2),
        objective="binary:logistic", random_state=RANDOM_STATE,
        n_jobs=-1, verbosity=0,
    )
