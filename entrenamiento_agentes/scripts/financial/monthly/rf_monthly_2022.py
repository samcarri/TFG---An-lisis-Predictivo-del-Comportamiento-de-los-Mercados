...
Random Forest para predicción a 1 mes (21 días) con datos desde 2022.

Output:
  scripts/financial/outputs/monthly/rf_monthly_2022_results.csv
  scripts/financial/outputs/monthly/rf_monthly_2022_plot.png
"""
...
OUTPUT_CSV  = "scripts/financial/outputs/monthly/rf_monthly_2022_results.csv"
OUTPUT_PLOT = "scripts/financial/outputs/monthly/rf_monthly_2022_plot.png"
MODEL_PATH  = "trained_models/rf_monthly_2022_model.joblib"
SCALER_PATH = "trained_models/rf_monthly_2022_scaler.joblib"
PREDICTION_HORIZON = 21
STEP        = 10
RANDOM_STATE = 42


def load_data(path, horizon=21, start_date="2022-01-01"):
    df = pd.read_csv(path)
    ...
    df["target_monthly"] = (df["future_close"] > df["Close"]).astype(int)
    df = df[:-horizon].copy()
    
    available = [c for c in df.columns if c not in {"date", "target", "target_monthly", "future_close"}]
    X = df[available].values
    y = df["target_monthly"].values
...
def evaluate(model, X, y, threshold, label=""):
    ...
    return {"label": label, "threshold": threshold, "accuracy": acc,
            "f1_weighted": f1, "recall_sube": rec_sube, "precision_sube": pre_sube,
            "recall_baja": rec_baja, "precision_baja": pre_baja, "roc_auc": auc,
            "horizon_days": PREDICTION_HORIZON}
...
def main():
    ...
    print("=" * 60)
    print(f"RANDOM FOREST MONTHLY 2022 — PREDICCIÓN A {PREDICTION_HORIZON} DÍAS")
    print("=" * 60)

    X, y, dates, features = load_data(DATA_PATH, horizon=PREDICTION_HORIZON, start_date="2022-01-01")
    ...
    axes[0].set_title(f"RF Monthly 2022 ({PREDICTION_HORIZON}d) — Feature Importance", fontweight="bold")
